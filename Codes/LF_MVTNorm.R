...
grad_mvt_norm <- function(x, s.2 = 1){
  d <- length
  return(x/(s.2))
}


LF_smplr_Mvt_Norm <- function(L = 10, eps = .01, d = 40, n = 1e4)
{
  ...
  for(k in 2:n)
  {
    ...
    q_prop <- qt[k-1,]
    # one half-Euler step
    p_prop <- p - eps/2 * grad_mvt_norm(q_prop)
    for(l in 1:L) 
    {
      q_prop <- q_prop + eps * p_prop
      if(l != L) p_prop <- p_prop - eps*grad_mvt_norm(q_prop)
    }
    # one last half-Euler step
    p_prop <- p_prop - eps/2 * grad_mvt_norm(q_prop)
    # Accept-reject
...
for(l in 1:grid.length.Ls){
  for(eps in 1:grid.length.eps){
    chain <- LF_smplr_Mvt_Norm(L = l, eps = EPSs[eps], n = n)
    if(chain[[2]] <= 2e-4){
...
data$Ess <- c(ess.heat.map.plt[1:grid.length.Ls,1:grid.length.eps])/n
